...
def save_data(
    ...
    os.makedirs(
        os.path.dirname(csv_file) if os.path.dirname(csv_file) else ".", exist_ok=True
    )
...
if __name__ == "__main__":
    ...
    combinations = list(
        itertools.product(T0_values, Tf_values, L_factors, coolingFunctions)
    )

    series_list = [("TS1.txt", 9), ("TS2.txt", 15), ("TS3.txt", 25), ("TS4.txt", 50)]
    ...
    for filename, k_segments in series_list:
        ...
        for T0, Tf, factor_L, (coolingFunction, function_name) in combinations:
            L_real = int(
                factor_L * vecindario
            )  # En vez de hacerlo arbitrario, el tamaño L se calcula con un factor del vecindario
            ...
            for i in range(repeticiones):
                ...
                print(
                    f"Config: T0={T0:<4.1f} | Tf={Tf:<5.3f} | L_factor={factor_L} (L_real={L_real}) --> MSE: {mse:.4f}"
                )
